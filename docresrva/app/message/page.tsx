...
const ChatRoom = () => {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      if (!activeUser) return; // Only fetch messages if an activeUser is set
      try {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_USER_BACKEND_URL}/chat?roomId=${activeUser}`, {
          withCredentials: true,
        });
        console.log(response.data);
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await axios.put(`${process.env.NEXT_PUBLIC_USER_BACKEND_URL}/chat`, {
        activeUser,
        message,
      }, { withCredentials: true });
          ...
          <div className="flex items-center justify-between p-4 bg-white shadow-md rounded-lg mb-4">
            <div className="flex items-center">
              <div className="w-12 h-12 rounded-full bg-gray-300 flex items-center justify-center mr-3">
                {selectedUser?.doctor?.profilePic ? (
                  <img
                    src={selectedUser?.doctor?.profilePic}
                    alt="Profile"
                    className="w-full h-full object-cover rounded-full"
                  />
                ) : (
                  selectedUser?.doctor?.name?.charAt(0)
                )}
              </div>
              <div>
                <span className="font-semibold">{selectedUser?.doctor?.name}</span>
                {/* Online status */}
